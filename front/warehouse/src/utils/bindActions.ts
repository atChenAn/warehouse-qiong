...
interface BindActions {
  (actions: NamespaceActions | any, dispatch: Dispatch<any>): NamespaceActions;
}
...
const bindActions: BindActions = (actions, dispatch) => {
  const namespace = compose<string>(head, keys)(actions);
...
interface BindActionsCollection {
  (...actionsCollection: NamespaceActions[]): (dispatch: Dispatch<any>) => any;
}
 ...
 * });
 */
const bindActionsCollection: BindActionsCollection = (
  ...actionsCollection
) => dispatch => {
